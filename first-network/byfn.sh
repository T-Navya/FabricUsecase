...
function networkUp() {
  ...
  if [ "${CERTIFICATE_AUTHORITIES}" == "true" ]; then
    ...
    export BYFN_CA1_PRIVATE_KEY=$(cd crypto-config/peerOrganizations/org1.pdt.com/ca && ls *_sk)
    export BYFN_CA2_PRIVATE_KEY=$(cd crypto-config/peerOrganizations/org2.pdt.com/ca && ls *_sk)
  fi
...
function upgradeNetwork() {
  if [[ "$IMAGETAG" == *"1.4"* ]] || [[ $IMAGETAG == "latest" ]]; then
    docker inspect -f '{{.Config.Volumes}}' orderer.pdt.com | grep -q '/var/hyperledger/production/orderer'
    if [ $? -ne 0 ]; then
    ...
    docker-compose $COMPOSE_FILES stop orderer.pdt.com
    docker cp -a orderer.pdt.com:/var/hyperledger/production/orderer $LEDGERS_BACKUP/orderer.pdt.com
    docker-compose $COMPOSE_FILES up -d --no-deps orderer.pdt.com

    for PEER in peer0.org1.pdt.com   peer0.org2.pdt.com ; do
...
function replacePrivateKey() {
  ...
  CURRENT_DIR=$PWD
  cd crypto-config/peerOrganizations/org1.pdt.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
